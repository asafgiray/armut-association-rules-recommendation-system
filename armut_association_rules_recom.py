...
df["Hizmet"] = [str(row[1]) + "_" + str(row[2]) for row in df.values] #row1 -> ServiceId değeri  row2->CategoryId değeri
df.head()
...
df.info()
df["CreateDate"] = pd.to_datetime(df["CreateDate"]) #datetime formatına çevirdik
df.head()
df["NewDate"] = df["CreateDate"].dt.strftime("%Y-%m") #CreateDate'nin sadece yıl ve ay değişkenlerini aldık
df.head()
